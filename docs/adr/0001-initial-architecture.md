...
remain usable locally before deployment, avoid autonomous external writes, and
keep its source datasets reproducible and authoritative outside AWS.
...
- The MVP exposes read-only tools and separates retrieved facts from generated
  recommendations. Any future external write requires preview, explicit
  authenticated approval, idempotency, and an audit record.
...
- A VPC, stronger default model, Knowledge Base, streaming, writes, or
